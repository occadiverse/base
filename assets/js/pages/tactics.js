        ...
        window.tacticalLiveEvent = null;
        window.tacticalLiveDirty = false;
        window.tacticalAppliedLiveSubs = window.tacticalAppliedLiveSubs || [];
        window.tacticalAppliedLiveMoves = window.tacticalAppliedLiveMoves || [];
        ...
        window.resetLiveMatchClock = async function() {
            ...
            const savedSubCount = (
                ...
                + (Array.isArray(match?.liveRoleChanges) ? match.liveRoleChanges.length : 0)
                + (Array.isArray(window.tacticalAppliedLiveRoleChanges) ? window.tacticalAppliedLiveRoleChanges.length : 0)
            );
        ...
        window.toggleLiveSessionLock = async function() {
            ...
            window.clearTacticalPendingSub({ restore: true });
        ...
        function serializeLiveRoleChanges(changes) {
            return (Array.isArray(changes) ? changes : []).map((change) => ({
                minute: String(change?.minute ?? ''),
                slotId: String(change?.slotId || '').trim(),
                fromRole: String(change?.fromRole || change?.slotId || '').trim(),
                toRole: String(change?.toRole || '').trim(),
                playerId: change?.playerId || ''
            })).filter((change) => change.slotId && change.toRole && change.fromRole !== change.toRole);
        }

        function cloneLiveSlotRoles(roles) {
            return { ...(roles || {}) };
        }

        function getLiveSlotRole(slotId, extras) {
            const key = String(slotId || '').trim();
            if (!key) return '';
            const overlay = extras && typeof extras === 'object' ? extras : window.liveSlotRoles;
            const mapped = overlay && overlay[key] ? String(overlay[key]).trim() : '';
            return mapped || key;
        }

        function applyLiveRoleChangeToState(change) {
            const slotId = String(change?.slotId || '').trim();
            const toRole = String(change?.toRole || '').trim();
            if (!slotId || !toRole) return;
            window.liveSlotRoles = window.liveSlotRoles || {};
            if (toRole === slotId) delete window.liveSlotRoles[slotId];
            else window.liveSlotRoles[slotId] = toRole;
        }

        function hydrateLiveSlotRolesFromChanges(changes) {
            window.liveSlotRoles = {};
            serializeLiveRoleChanges(changes).forEach((change) => applyLiveRoleChangeToState(change));
        }

        function clearLiveLineupPos(posId) {
            if (!posId || !window.liveLineup) return;
            window.liveLineup[posId] = null;
        }

        function applyLiveSubToLineupState(sub) {
            if (!sub?.posId || !sub?.inId) return;
            ...
            if (fillPlayer && fillFromPos) {
                ...
                if (fillFromPos !== inPosId) {
                    const displaced = window.liveLineup?.[inPosId] || null;
                    window.liveLineup[fillFromPos] = displaced;
                    clearLiveLineupPos(inPosId);
                }
            } else {
                clearLiveLineupPos(inPosId);
            }
        ...
        function applyLiveMoveToLineupState(move) {
            const aPos = move?.aPos;
            const bPos = move?.bPos;
            if (!aPos || !bPos || aPos === bPos) return;
            const playerA = window.liveLineup?.[aPos] || null;
            const playerB = window.liveLineup?.[bPos] || null;
            window.liveLineup[aPos] = playerB;
            window.liveLineup[bPos] = playerA;
        }
        ...
        function resolvePlayerKeyFromRef(ref) {
            if (!ref) return '';
            const player = typeof window.findPlayerByRef === 'function'
        ...
        function replayLivePositionTimeline(match, durationMinutes) {
            ...
            const closeStint = (key, atMinute) => {
                ...
                if (!state) return;
                const mins = Math.max(0, atMinute - state.since);
            ...
            const leavePos = (posId, atMinute) => {
                const key = board[posId];
                if (!key) return null;
                delete board[posId];
            ...
            const occupyPos = (key, posId, atMinute) => {
                if (!key || !posId) return;
                if (board[posId] && board[posId] !== key) {
                    leavePos(posId, atMinute);
                }
                closeStint(key, atMinute);
                board[posId] = key;
                const roleId = slotRoles[posId] || posId;
            ...
            const events = [];
            serializeLiveSubstitutions(match?.liveSubstitutions).forEach((sub) => {
                events.push({
                    type: 'sub',
                    minute: parseLiveSubMinuteValue(sub.minute),
                    sub
                });
            });
            serializeLiveLineupMoves(match?.liveLineupMoves).forEach((move) => {
                events.push({
                    type: 'move',
                    minute: parseLiveSubMinuteValue(move.minute),
                    move
                });
            });
            serializeLiveRoleChanges(match?.liveRoleChanges).forEach((change) => {
                events.push({
                    type: 'role',
                    minute: parseLiveSubMinuteValue(change.minute),
                    change
                });
            });
            // Same minute: sub, then role change, then rokering. Stable within type.
            const liveEventTypeOrder = { sub: 0, role: 1, move: 2 };
            events.sort((a, b) => {
                if (a.minute !== b.minute) return a.minute - b.minute;
                return (liveEventTypeOrder[a.type] ?? 9) - (liveEventTypeOrder[b.type] ?? 9);
            });
            ...
            events.forEach((event) => {
                ...
                if (event.type === 'sub') {
                    ...
                    const fillFromPos = sub.fillFromPos || '';
                    const fillKey = resolvePlayerKeyFromRef(sub.fillId) || (fillFromPos ? board[fillFromPos] : '');
                    if (fillFromPos && fillKey) {
                        if (board[fillFromPos] === fillKey) delete board[fillFromPos];
                        closeStint(fillKey, minute);
                        occupyPos(fillKey, outPosId, minute);
                        if (fillFromPos !== inPosId) {
                            const displacedKey = board[inPosId] || null;
                            if (displacedKey) {
                                delete board[inPosId];
                                closeStint(displacedKey, minute);
                                occupyPos(displacedKey, fillFromPos, minute);
                            }
                        }
                    } else {
                        leavePos(inPosId, minute);
                    }
                    occupyPos(inKey, inPosId, minute);
                    return;
                }
                ...
                if (event.type === 'role') {
                    ...
                    const toRole = String(change.toRole || '').trim();
                    if (!slotId || !toRole) return;
                    const key = board[slotId];
                    if (key) closeStint(key, minute);
                    if (toRole === slotId) delete slotRoles[slotId];
                    else slotRoles[slotId] = toRole;
                    if (key) {
                        board[slotId] = key;
                        open.set(key, { posId: toRole, slotId, since: minute });
                    }
                    return;
                }
                ...
                const bPos = move.bPos;
                if (!aPos || !bPos || aPos === bPos) return;
                const aKey = board[aPos] || resolvePlayerKeyFromRef(move.aId);
                const bKey = board[bPos] || resolvePlayerKeyFromRef(move.bId);
                leavePos(aPos, minute);
                leavePos(bPos, minute);
                if (bKey) occupyPos(bKey, aPos, minute);
                if (aKey) occupyPos(aKey, bPos, minute);
            });
        ...
        function computeLivePositionMinutes(match, durationMinutes) {
            ...
            totals.forEach((posMap, key) => {
                const row = {};
                posMap.forEach((mins, posId) => {
                    if (mins > 0) row[posId] = mins;
                });
                if (Object.keys(row).length) result[key] = row;
            });
            return result;
        }
        ...
        function hydrateLiveSubstitutionsFromMatch(match) {
            ...
            const savedMoves = Array.isArray(match?.liveLineupMoves) ? match.liveLineupMoves : [];
            const savedRoleChanges = Array.isArray(match?.liveRoleChanges) ? match.liveRoleChanges : [];
            window.tacticalAppliedLiveSubs = serializeLiveSubstitutions(saved);
            window.tacticalAppliedLiveMoves = serializeLiveLineupMoves(savedMoves);
            window.tacticalAppliedLiveRoleChanges = serializeLiveRoleChanges(savedRoleChanges);
            ...
            hydrateLiveSlotRolesFromChanges(window.tacticalAppliedLiveRoleChanges);
            syncLiveLineupToTactical();
        }
        ...
        window.persistLivePlayingTime = async function(options = {}) {
            ...
            const liveLineupMoves = serializeLiveLineupMoves(window.tacticalAppliedLiveMoves || []);
            const liveRoleChanges = serializeLiveRoleChanges(window.tacticalAppliedLiveRoleChanges || []);
            const lastSubMinute = liveSubstitutions.reduce(
                (max, sub) => Math.max(max, parseLiveSubMinuteValue(sub.minute)),
                0
            );
            const lastMoveMinute = liveLineupMoves.reduce(
                (max, move) => Math.max(max, parseLiveSubMinuteValue(move.minute)),
                0
            );
            const lastRoleMinute = liveRoleChanges.reduce(
                (max, change) => Math.max(max, parseLiveSubMinuteValue(change.minute)),
                0
            );
            const duration = options.durationMinutes != null && options.durationMinutes !== ''
                ...
                : Math.max(clockMinute, lastSubMinute, lastMoveMinute, lastRoleMinute);

            match.liveSubstitutions = liveSubstitutions;
            match.liveLineupMoves = liveLineupMoves;
            match.liveRoleChanges = liveRoleChanges;
        ...
        window.applyTacticalLineupReadOnlyState = function() {
            ...
            const pickingOut = Boolean(event?.inPlayer && !event?.editorOpen);
            const eventActive = Boolean(
                (event && !event.inPlayer)
                || (event?.moves?.length)
                || (event?.roleChanges?.length)
            );
            ...
            if (pitch) {
                ...
                pitch.classList.toggle('is-sub-targeting', liveMatch && !liveLocked && pickingOut);
                pitch.classList.toggle('is-sub-phase-out', liveMatch && !liveLocked && pickingOut);
                pitch.classList.toggle('is-sub-phase-in', false);
                pitch.classList.toggle('is-sub-phase-fill', false);
                pitch.classList.toggle(
                    'is-swap-targeting',
                    liveMatch && !liveLocked && swapMode && Boolean(pendingSwapPos)
                );
            }
            ...
            document.querySelectorAll('.player-node').forEach(node => {
                ...
                node.classList.toggle('is-sub-target', liveMatch && !liveLocked && pickingOut && filled);
                node.classList.toggle('is-sub-out', liveMatch && !liveLocked && pendingOutPos === posId);
                node.classList.toggle('is-sub-in-pos', false);
                node.classList.toggle('is-sub-hole', false);
        ...
        function refreshTacticalLiveSubsLog() {
            ...
            const displayMatch = {
                ...
                liveRoleChanges: Array.isArray(window.tacticalAppliedLiveRoleChanges)
                    ? window.tacticalAppliedLiveRoleChanges
                    : (match.liveRoleChanges || [])
            };
        ...
        function cloneLiveLineupMap(lineup) {
            const out = {};
            Object.keys(lineup || {}).forEach((posId) => {
                out[posId] = lineup[posId] || null;
            });
            return out;
        }

        function hideLiveEventPanel() {
            const panel = document.getElementById('tactical-live-sub-panel');
            if (!panel) return;
            panel.classList.add('hidden');
            panel.innerHTML = '';
        ...
        function getLiveEventPlayerLabel(playerOrRef) {
            if (!playerOrRef) return '—';
            if (typeof playerOrRef === 'object') {
                return getTacticalLivePlayerLastName(playerOrRef) || playerOrRef.navn || '—';
            }
            const player = typeof window.findPlayerByRef === 'function'
                ? window.findPlayerByRef(playerOrRef)
                : null;
            if (player) return getTacticalLivePlayerLastName(player) || player.navn || String(playerOrRef);
            return String(playerOrRef);
        }

        function liveEventHasStagedChanges(event = window.tacticalLiveEvent) {
            return Boolean(
                (event?.moves && event.moves.length)
                || (event?.roleChanges && event.roleChanges.length)
            );
        }

        function createEmptyLiveEvent() {
            return {
                snapshotLineup: cloneLiveLineupMap(window.liveLineup),
                snapshotRoles: { ...(window.liveRoles || {}) },
                snapshotSlotRoles: cloneLiveSlotRoles(window.liveSlotRoles),
                inPlayer: null,
                sub: null,
                moves: [],
                roleChanges: [],
        ...
        function ensureLiveEventForRokering() {
            ...
            window.tacticalLiveEvent = createEmptyLiveEvent();
        ...
        function buildLiveEventSummaryHtml(event) {
            ...
            const moveBits = (event.moves || []).map((move) => {
                ...
                const aPos = getLivePosBadgeLabel(move.aPos);
            ...
            const roleBits = (event.roleChanges || []).map((change) => {
                const playerName = getLiveEventPlayerLabel(change.playerId || window.liveLineup?.[change.slotId]);
                const fromPos = getLivePosBadgeLabel(change.fromRole || change.slotId);
                const toPos = getLivePosBadgeLabel(change.toRole);
                return `${escapeTacticalHtml(fromPos)} → ${escapeTacticalHtml(toPos)}${playerName ? ` (${escapeTacticalHtml(playerName)})` : ''}`;
            });
            const items = [];
            if (moveBits.length) {
            ...
            if (roleBits.length) {
                items.push(`<li><strong>Gjør om:</strong> ${roleBits.join(' · ')}</li>`);
            }
            if (!items.length) {
                items.push('<li class="is-muted">Trykk to spillere på banen for å bytte plass, eller Gjør om til.</li>');
            }
        ...
        function renderLiveEventPanel() {
            ...
            const roleCount = event.roleChanges?.length || 0;
            const moveCount = event.moves?.length || 0;
            const finishTitle = roleCount && !moveCount
                ? 'Fullfør gjør om'
                : (moveCount && !roleCount ? 'Fullfør rokering' : 'Fullfør');
            const pickerSlot = event.rolePickerSlot || window.tacticalPendingSwapPos || '';
            const pickerRole = getLiveSlotRole(pickerSlot);
            const pickerLabel = getLivePosBadgeLabel(pickerRole || pickerSlot);

            if (event.rolePickerOpen && pickerSlot) {
                const chips = TACTICAL_SLOT_ROLE_OPTIONS.map((opt) => {
                    const active = opt.id === pickerRole;
                    return `
                        <button
                            type="button"
                            class="bsk-btn bsk-btn-chip tactical-live-role-chip${active ? ' is-active' : ''}"
                            data-live-event="pick-role"
                            data-role-id="${escapeTacticalHtml(opt.id)}"
                            ${active ? 'disabled' : ''}
                        >${escapeTacticalHtml(opt.label)}</button>
                    `;
                }).join('');
                panel.innerHTML = `
                    <div class="tactical-live-event-bar">
                        <p class="tactical-live-event-copy">Gjør om ${escapeTacticalHtml(pickerLabel)} til</p>
                        <div class="tactical-live-role-grid" role="group" aria-label="Velg ny posisjon">${chips}</div>
                        <div class="tactical-live-event-actions">
                            <button type="button" class="bsk-btn bsk-btn-secondary tactical-bench-btn" data-live-event="role-back">Tilbake</button>
                            <button type="button" class="bsk-btn bsk-btn-secondary tactical-bench-btn" data-live-event="abort">Avbryt</button>
                        </div>
                    </div>
                `;
            } else if (event.dialogOpen && canFinish) {
                const defaultMinute = event.planMinute !== undefined && event.planMinute !== ''
                    ? String(event.planMinute).trim().replace(/'$/, '')
                    : resolveLiveSubMinute();
                panel.innerHTML = `
                    <div class="tactical-live-event-dialog" role="dialog" aria-labelledby="tactical-live-event-dialog-title">
                        ...
                        <label class="tactical-live-event-minute">
                            ...
                            <input
                                ...
                                max="130"
                                step="1"
                                id="tactical-live-event-minute-input"
                                value="${escapeTacticalHtml(defaultMinute)}"
                            >
                        </label>
                        <div class="tactical-live-event-actions">
                ...
                panel.innerHTML = `
                    <div class="tactical-live-event-bar">
                        <p class="tactical-live-event-copy">${roleCount ? 'Gjør om klar — trykk Ferdig' : 'Rokering klar — fortsett eller trykk Ferdig'}</p>
                        ${buildLiveEventSummaryHtml(event)}
                        <div class="tactical-live-event-actions">
                ...
                panel.innerHTML = `
                    <div class="tactical-live-event-bar">
                        <p class="tactical-live-event-copy">Rokering — velg to spillere, eller gjør om plassen</p>
                        <div class="tactical-live-event-actions">
                            <button type="button" class="bsk-btn bsk-btn-primary tactical-bench-btn" data-live-event="role-open">Gjør om til</button>
                            <button type="button" class="bsk-btn bsk-btn-secondary tactical-bench-btn" data-live-event="abort">Avbryt</button>
                        </div>
                    </div>
                `;
            ...
            panel.querySelectorAll('[data-live-event]').forEach((btn) => {
                btn.addEventListener('click', () => {
                    const action = btn.getAttribute('data-live-event');
                    if (action === 'abort') {
                        window.clearTacticalPendingSub({ restore: true });
                        refreshTacticalLiveBoard();
                        return;
                    ...
                    if (action === 'finish') {
                        window.openLiveEventFinishDialog();
                        return;
                    }
                    if (action === 'back') {
                        if (window.tacticalLiveEvent) window.tacticalLiveEvent.dialogOpen = false;
                        updateLiveSubStatusMessage();
                        renderLiveEventPanel();
                        return;
                    }
                    if (action === 'role-open') {
                        window.openLiveRolePicker();
                        return;
                    }
                    if (action === 'role-back') {
                        if (window.tacticalLiveEvent) {
                            window.tacticalLiveEvent.rolePickerOpen = false;
                        }
                        updateLiveSubStatusMessage();
                        renderLiveEventPanel();
                        return;
                    ...
                    if (action === 'pick-role') {
                        window.stageLiveRoleChange(btn.getAttribute('data-role-id'));
                        return;
                    }
                    if (action === 'confirm') {
                        const input = document.getElementById('tactical-live-event-minute-input');
                        window.commitLiveEvent(input?.value);
                    }
                });
            });

            const minuteInput = document.getElementById('tactical-live-event-minute-input');
            if (minuteInput) {
                minuteInput.focus();
                minuteInput.select();
            }
        ...
        window.clearTacticalPendingSub = function(options = {}) {
            ...
            if (shouldRestore) {
                ...
                window.liveRoles = { ...(event.snapshotRoles || {}) };
                window.liveSlotRoles = cloneLiveSlotRoles(event.snapshotSlotRoles);
                syncLiveLineupToTactical();
            }

            // Lukk create-live-modal uten å trigge cancel-hook på nytt.
            const editState = window._matchSubsLogEditState;
            if (editState?.mode === 'create-live') {
                editState.saved = true;
        ...
        function updateLiveSubStatusMessage() {
            ...
            if (event.dialogOpen) {
                setLivePlayingTimeStatus(
                    (event.roleChanges?.length && !event.moves?.length)
                        ? 'Bekreft gjør om med kampminutt'
                        : 'Bekreft rokering med kampminutt',
                    'pending'
                );
                return;
            }
            if (event.rolePickerOpen) {
                setLivePlayingTimeStatus('Velg ny posisjon', 'pending');
                return;
            }
            if (window.tacticalPendingSwapPos) {
                setLivePlayingTimeStatus('Velg spiller å bytte plass med, eller Gjør om til', 'pending');
                return;
            }
            const moveCount = event.moves?.length || 0;
            const roleCount = event.roleChanges?.length || 0;
            if (roleCount && !moveCount) {
                setLivePlayingTimeStatus(`${roleCount} gjør om. Trykk Ferdig.`, 'pending');
                return;
            }
            if (moveCount) {
        ...
        window.beginTacticalLiveSub = function(playerId) {
            ...
            if (window.tacticalPendingSubIn?.id === player.id || window.tacticalLiveEvent?.inPlayer?.id === player.id) {
                window.clearTacticalPendingSub({ restore: true });
                refreshTacticalLiveBoard();
                return;
            }
            ...
            window.tacticalPendingSwapPos = null;
            window.tacticalLiveEvent = createEmptyLiveEvent();
            window.tacticalLiveEvent.inPlayer = player;
        ...
        function openLiveSubEditorForOut(outPosId) {
            const event = window.tacticalLiveEvent;
            const inPlayer = event?.inPlayer;
            if (!event || !inPlayer || !outPosId) return false;
            if (window.isLiveSessionLocked()) {
                setLivePlayingTimeStatus('Live er låst', 'error');
                return false;
            }
            if (typeof window.openMatchSubsLogCreateFromLive !== 'function') {
                alert('Bytte-dialog er ikke tilgjengelig.');
                return false;
            }

            const outPlayer = window.liveLineup?.[outPosId] || null;
            if (!outPlayer) {
                alert(`Ingen spiller på ${outPosId} å bytte ut.`);
                return false;
            }

            const alreadyOnPitch = Object.entries(window.liveLineup || {}).some(([, player]) => (
                player && (player.id === inPlayer.id || player.navn === inPlayer.navn)
            ));
            if (alreadyOnPitch) {
            ...
            event.editorOpen = true;
            event.sub = null;
            event.moves = [];
            window.tacticalPendingSubOutPos = outPosId;
            window.tacticalPendingSwapPos = null;
            updateLiveSubStatusMessage();
            renderLiveEventPanel();
            window.applyTacticalLineupReadOnlyState();

            const opened = window.openMatchSubsLogCreateFromLive({
                matchId: match.id,
                outId: getTacticalLivePlayerRef(outPlayer),
                inId: getTacticalLivePlayerRef(inPlayer),
                posId: outPosId,
                inPosId: outPosId,
        ...
        function stageLiveEventMove(aPos, bPos) {
            ...
            // Ikke tillat pitch-rokering midt i et bytte — fill-kjede håndteres i dialogen.
            if (window.tacticalLiveEvent?.inPlayer) return false;
        ...
        window.openLiveRolePicker = function() {
            if (window.isLiveSessionLocked()) {
                setLivePlayingTimeStatus('Live er låst', 'error');
                return false;
            }
            const event = ensureLiveEventForRokering();
            if (event.inPlayer || event.editorOpen) return false;
            const slotId = window.tacticalPendingSwapPos || event.rolePickerSlot || '';
            if (!slotId || !window.liveLineup?.[slotId]) {
                alert('Velg en spiller på banen først.');
                return false;
            }
            event.rolePickerOpen = true;
            event.rolePickerSlot = slotId;
            event.dialogOpen = false;
            updateLiveSubStatusMessage();
            renderLiveEventPanel();
            refreshTacticalLiveBoard();
        ...
        window.stageLiveRoleChange = function(toRoleId) {
            if (window.isLiveSessionLocked()) {
                setLivePlayingTimeStatus('Live er låst', 'error');
                return false;
            }
            const event = ensureLiveEventForRokering();
            const slotId = event.rolePickerSlot || window.tacticalPendingSwapPos || '';
            const toRole = String(toRoleId || '').trim();
            if (!slotId || !toRole || !TACTICAL_POSITIONS.includes(toRole)) return false;
            if (!window.liveLineup?.[slotId]) {
                alert('Plassen må ha en spiller.');
                return false;
            }
            const fromRole = getLiveSlotRole(slotId);
            if (fromRole === toRole) return false;

            applyLiveRoleChangeToState({ slotId, toRole });
            event.roleChanges = [...(event.roleChanges || []), {
                minute: '',
                slotId,
                fromRole,
                toRole,
                playerId: getTacticalLivePlayerRef(window.liveLineup[slotId])
            }];
            event.rolePickerOpen = false;
            event.rolePickerSlot = '';
            window.tacticalPendingSwapPos = null;
            window.tacticalLiveDirty = true;
            updateLiveSubStatusMessage();
            renderLiveEventPanel();
        ...
        window.commitLiveEvent = function(minuteValue) {
            ...
            (event.roleChanges || []).forEach((change) => {
        ...
        window.commitLiveSubFromEditor = async function(subRecord) {
            if (!subRecord || window.isLiveSessionLocked()) {
                setLivePlayingTimeStatus('Live er låst', 'error');
                return false;
            }

            const serialized = serializeLiveSubstitutions([subRecord])[0];
            if (!serialized) return false;

            (serialized.outRoles || []).forEach((slot) => {
                if (!slot) return;
                window.liveRoles[slot] = serialized.inId;
            });

            applyLiveSubToLineupState(serialized);
            syncLiveLineupToTactical();
            window.tacticalLiveDirty = true;
            window.tacticalAppliedLiveSubs = [
                ...(window.tacticalAppliedLiveSubs || []),
            ...
            if (typeof window.persistLivePlayingTime === 'function') {
                try {
                    await window.persistLivePlayingTime({ commitMinutes: false });
                } catch (error) {
                    console.error('Kunne ikke lagre Live-bytte:', error);
                    setLivePlayingTimeStatus('Kunne ikke lagre bytte', 'error');
                    return false;
                }
            }
            return true;
        ...
        window.cancelLiveSubFromEditor = function() {
            const event = window.tacticalLiveEvent;
            if (!event?.editorOpen && !event?.inPlayer) return;
            window.clearTacticalPendingSub({ restore: false });
            refreshTacticalLiveBoard();
        };

        window.applyLiveSubstitution = function(outPosId, inPlayer, options = {}) {
            if (!window.isTacticalLiveMatchMode() || !outPosId || !inPlayer) return false;
            if (window.isLiveSessionLocked()) {
                setLivePlayingTimeStatus('Live er låst', 'error');
                return false;
            }
            ...
            const event = window.tacticalLiveEvent;
            const sameIn = Boolean(event?.inPlayer && (
                event.inPlayer.id === inPlayer.id || event.inPlayer.navn === inPlayer.navn
            ));
            if (!event || !sameIn || event.editorOpen) {
                window.clearTacticalPendingSub({ restore: true });
                window.tacticalPendingSwapPos = null;
                window.tacticalLiveEvent = createEmptyLiveEvent();
                window.tacticalLiveEvent.inPlayer = inPlayer;
                window.tacticalPendingSubIn = inPlayer;
                window.tacticalPendingSubOutPos = null;
            }
            if (window.tacticalLiveEvent && options.minute != null && options.minute !== '') {
                window.tacticalLiveEvent.planMinute = String(options.minute).trim().replace(/'$/, '');
            }
            return openLiveSubEditorForOut(outPosId);
        };
        ...
        window.renderBench = function() {
            ...
            benchPlayers.forEach(p => {
                ...
                const isPending = Boolean(window.tacticalLiveEvent?.inPlayer && (
                    window.tacticalLiveEvent.inPlayer.id === p.id
                    || window.tacticalLiveEvent.inPlayer.navn === p.navn
                ));
                const pendingHint = window.tacticalLiveEvent?.editorOpen
                    ? 'Fullfør i dialogen'
                    : 'Velg hvem som går ut';
                ...
                const appliedInSub = findLatestAppliedLiveSubForPlayer(p, 'in');
                const outMinuteRaw = appliedOutSub?.minute
                    ? String(appliedOutSub.minute).trim().replace(/'$/, '')
                    : '';
                const inMinuteRaw = appliedInSub?.minute
                    ? String(appliedInSub.minute).trim().replace(/'$/, '')
                    : '';
                const outTimeHtml = outMinuteRaw
                    ? `<div class="tactical-bench-sub-times">
                            ...
                            ${inMinuteRaw
                                ? `<div class="tactical-bench-swap-meta is-in-time" title="Byttet inn ${escapeTacticalHtml(inMinuteRaw)}'">
                                        <span class="tactical-bench-plan-minute">INN</span>
                                        <span class="tactical-bench-plan-pos">${escapeTacticalHtml(inMinuteRaw)}'</span>
                                   </div>`
                                : ''}
                       </div>`
                    : '';
                ...
                const statusBits = [
                    ...
                    !liveLocked && isPending ? `<span class="tactical-bench-pending">${escapeTacticalHtml(pendingHint)}</span>` : ''
                ].filter(Boolean).join('');
                ...
                div.innerHTML = `
                    ...
                    <div class="tactical-bench-buttons">
                        ...
                        <button
                            ...
                            title="${liveLocked ? 'Live er låst' : (isPending ? 'Avbryt bytte' : 'Bytt inn – velg hvem som går ut')}"
                            ${liveLocked ? 'disabled' : ''}
        ...
        window.renderNodeVisually = function(playerObj, posId) {
            ...
            const displayRole = window.isTacticalLiveMatchMode()
                ? getLiveSlotRole(posId)
                : posId;
            const posBadge = getTacticalLivePosBadge(displayRole);
            const roleChanged = window.isTacticalLiveMatchMode() && displayRole !== posId;
            node.classList.toggle('is-slot-role-changed', roleChanged);
        ...
        window.openPlayerSelect = function(posId) {
            if (window.isTacticalLiveMatchMode()) {
                ...
                if (window.tacticalLiveEvent?.inPlayer && !window.tacticalLiveEvent.editorOpen) {
                    if (!window.liveLineup?.[posId]) {
                        alert(`Ingen spiller på ${posId} å bytte ut.`);
                        return;
                    }
                    openLiveSubEditorForOut(posId);
                    return;
                }

                if (window.tacticalLiveEvent?.editorOpen) return;
                if (window.tacticalLiveEvent?.rolePickerOpen) return;
